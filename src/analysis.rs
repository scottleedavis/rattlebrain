...
fn handle_frames(data: &Value, filename: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Write CSV header row
    writeln!(file, "Delta,ActorID,Component,Detail,Value")?;

    // Define a stable default value for the array
    let empty_array: Vec<Value> = vec![];
    let empty_value_array = Value::Array(empty_array.clone());

    // Navigate to /content/frames
    let frames = data
        .pointer("/content/frames")
        .unwrap_or(&empty_value_array);
